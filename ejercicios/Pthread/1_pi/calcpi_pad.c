...
void *calculatePi(void *arg)
{
    ...
    do
    {
        piTotal[threadId * PAD] = piTotal[threadId * PAD] + (double)(4.0 / ((initIteration * 2) + 1));
        initIteration++;
        piTotal[threadId * PAD] = piTotal[threadId * PAD] - (double)(4.0 / ((initIteration * 2) + 1));
        initIteration++;
...
int main(int argc, char *argv[])
{
    ...
    piTotal = malloc(nThreads * sizeof(double) * PAD);
