...
void getOpticalFlow(uchar *, uchar *, OpticalVector *, int, int, int = 3);
...
void writeInform(char *, int, int, int, int, timeval, timeval);
...
timeval interpolateVideo(VideoCapture, char *, char *, int = 0, bool = false);
...
int main(int argc, char **argv)
{
    ...
    if (argc < R_ARGS + 1)
    {
        printf("Usage: ./MotionInterpolation <Path_to_files> <Load_video_name> <Save_video_name> <Frames to render (0 - all)> <Export_frames ( 0:false, 1:true )> \n");
        return -1;
    }
    // Get the paths
    path = argv[1];
    ...
    runtime = interpolateVideo(loadVideo, path, saveName, framesRend, expFrames);
    ...
    fprintf(fp, "%d,%d,%ld.%06ld\n", (int)loadVideo.get(CAP_PROP_FRAME_WIDTH), (int)loadVideo.get(CAP_PROP_FRAME_HEIGHT), (long int)runtime.tv_sec, (long int)runtime.tv_usec);
...
void getOpticalFlow(uchar *frame1, uchar *frame2, OpticalVector *optFlow, int width, int height, int channels)
{
...
void interpolateFrames(uchar *frame1, uchar *frame2, uchar *resFrame, OpticalVector *optFlow, int width, int height, int channels)
{
    ...
    getOpticalFlow(frame1, frame2, optFlow, width, height, channels);
...
void writeInform(char *path, int width, int height, int iterFrame, int frameCount, timeval tval_result, timeval runtime)
{
    // Declare the FILE to write the times
    FILE *fp;
    ...
    fprintf(fp, "%d,%d,%d,%f,%ld.%06ld\n", iterFrame, width, height, framesPSec, (long int)tval_result.tv_sec, (long int)tval_result.tv_usec);
    fclose(fp);
}
...
timeval interpolateVideo(VideoCapture loadVideo, char *path, char *saveName, int framesRend, bool expFrames)
{
    // Declare the variables for time measurement
    ...
    while (loadVideo.read(newFrame))
    {
        ...
        else if (iterFrame <= frameCount)
        {
            ...
            interpolateFrames(uFrameOld, uFrameNew, interFrame, optFlow, width, height, channels);
            ...
            writeInform(path, width, height, iterFrame, frameCount, tval_result, runtime);
